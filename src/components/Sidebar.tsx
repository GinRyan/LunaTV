          ...
          <div className='flex h-full flex-col'>
            ...
            <nav className='px-2 mt-4 space-y-1'>
              <Link
                ...
                onClick={() => setActive('/')}
                data-active={active === '/'}
                className={`group flex items-center rounded-lg px-2 py-2 pl-4 text-gray-700 hover:bg-gray-100/30 hover:text-green-600 data-[active=true]:bg-green-500/20 data-[active=true]:text-green-700 font-medium transition-colors duration-200 min-h-[40px] dark:text-gray-300 dark:hover:text-green-400 dark:data-[active=true]:bg-green-500/10 dark:data-[active=true]:text-green-400 ${isCollapsed ? 'w-full max-w-none mx-0 justify-center' : 'mx-0'
                  } gap-3 justify-start`}
              >
                <div className={`${isCollapsed ? 'w-10 h-10 flex items-center justify-center' : 'w-4 h-4 flex items-center justify-center'}`}>
                  <Home className={`${isCollapsed ? 'h-6 w-6' : 'h-4 w-4'} text-gray-500 group-hover:text-green-600 data-[active=true]:text-green-700 dark:text-gray-400 dark:group-hover:text-green-400 dark:data-[active=true]:text-green-400`} />
                </div>
                {!isCollapsed && (
              ...
              >
                <div className={`${isCollapsed ? 'w-10 h-10 flex items-center justify-center' : 'w-4 h-4 flex items-center justify-center'}`}>
                  <Search className={`${isCollapsed ? 'h-6 w-6' : 'h-4 w-4'} text-gray-500 group-hover:text-green-600 data-[active=true]:text-green-700 dark:text-gray-400 dark:group-hover:text-green-400 dark:data-[active=true]:text-green-400`} />
                </div>
                {!isCollapsed && (
                  <span className='whitespace-nowrap transition-opacity duration-200 opacity-100'>
                    搜索
              ...
              <div className='space-y-1'>
                {menuItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`${isCollapsed ? 'w-10 h-10 flex items-center justify-center' : 'w-4 h-4 flex items-center justify-center'}`}>
                        <Icon className={`${isCollapsed ? 'h-6 w-6' : 'h-4 w-4'} text-gray-500 group-hover:text-green-600 data-[active=true]:text-green-700 dark:text-gray-400 dark:group-hover:text-green-400 dark:data-[active=true]:text-green-400`} />
                      </div>
                      {!isCollapsed && (
                        <span className='whitespace-nowrap transition-opacity duration-200 opacity-100'>
                          {item.label}
